...
그러나 Vanishing Gradient가 발생하는 근본적인 원인은 Chain Rule에서 일어나기 때문에 위의 마이너한 변경들로는 이 현상을 방지할 수 없었다. 그렇기 떄문에 모델 구조 자체를 변경해야 하는 결론에 이르렀고, 이 때문에 고안된 모델이 LSTM과 GRU이다.
