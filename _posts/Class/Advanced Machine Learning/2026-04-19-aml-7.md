...
AdaBoost에서는 Binary Classification 문제를 다룬다. 즉, $y_i \in \left{ -1,1 \right}$, 각각의 Weak Learner $h(x) \in \left{ -1,1 \right}$를 만족한다. 또한 손실함수는 Exponential Loss를 사용한다. 즉, $l(H)=\sum\limits_{i=1}^{n}e^{-y_i H(x_i)}$이다.
