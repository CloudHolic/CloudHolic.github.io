...
$$p_X(x;\theta)=p_Z(f_{\theta}^{-1}(x))\prod_{m=1}^{M}\vert\det\left(\dfrac{\partial (f_{\theta}^m)^{-1}(z_m)}{\partial z_m}\right)\vert$$
...
$$\max_{\theta}\log p_X(D;\theta)=\sum_{x\in D}\left[\log p_Z(f_{\theta}^{-1}(x))+\sum_{m=1}^{M}\log\vert\det\left(\dfrac{\partial f_{\theta}^{-1}(x)}{\partial x}\right)\vert\right]$$
