...
위에서 생성모델은 분포를 학습하는 모델이라는 것을 알았다. 그러면 이 분포를 학습한다는게 어떤 의미인지 Direct Approach의 한 방법인 Density-based Model을 통해 알아보자. 이 방법은 이름에서도 알 수 있듯 우리가 학습해야 할 분포 $p(x)$의 PDF가 명확히 존재한다는 가정 하에 진행된다.
