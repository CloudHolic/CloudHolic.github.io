...
**BQP**(Bounded-error Quantum Polynomial time)는 양자 컴퓨터가 다항식 시간 내에 최대 $k$의 확률로 잘못된 답을 제시하는 문제들의 집합을 뜻한다. 이 오답률 $k$는 $0 < k < \dfrac{1}{2}$를 만족하기만 하면 어느 값이든 상관은 없으나 보통 $\dfrac{1}{3}$으로 생각한다. 이에 대응되는 고전 알고리즘에서의 복잡도 분류는 **BPP**이다.  
