...
이제 공리 A4를 $v$에 적용해보자. 공리 A4의 의미는 "혼합 분포를 $\lambda$로 변환할 때, 각 구성 요소를 먼저 변환한 결과들로부터 만들어져야 한다."는 뜻이다. 그리고 $v = \dfrac{1}{2} \cdot \delta_{\lbrace a, b \rbrace} + \dfrac{1}{2} \cdot \delta_{\lbrace c, d \rbrace}$로 정의되었었다.
