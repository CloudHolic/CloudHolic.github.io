...
다시 모나드에 관한 것을 잠깐 잊어버리고, 이번엔 다음과 같은 상황을 생각해보자. 여러 데이터를 한 컨텍스트에 저장했다. 그리고 이 데이터를 왼쪽에서부터 하나씩 합친 결과물을 얻고 싶다. 물론 합치는 방법은 자유다.
...
물론 `foldMap`같은게 있으니 Foldable은 Functor이면 좀 편하긴 하지만, 어쨌건 이론상으론 둘은 무관하기 때문에 Functor가 아닌 Foldable 타입 같은 것도 얼마든지 존재할 수 있다. 물론 모나드이면서 Foldable인 타입도 얼마든지 있고, 오히려 이쪽이 더 자연스럽다. 다만 MonadPlus와는 다르게 기존 모나드의 연산인 `return`과 `bind`와 결합해서 새로운 법칙을 만들어내지는 않기 때문에 이 타입에 대한 별도의 이름을 지을 필요를 느끼지 못한 듯 하다.
