...
모나드에 대한 것은 잠깐 잊고, `Option`, `List` 등의 컨텍스트에 대해 생각해보자. 이들은 데이터 앞에 붙어서 추가적인 정보를 제공해준다. 그럼 이러한 컨텍스트가 기본적으로 가져야 할 기능은 뭐가 있을까? 한 가지만 꼽자면 외부의 연산자를 이용할 수 있게 해주는 기능일 것이다. 이것조차 없으면 우리는 이 컨텍스트를 유지한 채로는 그 어떠한 계산도 수행할 수 없기 때문이다. 반드시 컨텍스트에서 꺼내야지만 연산을 할 수가 있다면 그 컨텍스트를 유지할 이유가 있겠는가?
...
이렇게 구현해두면 그럭저럭 실패 상황에 대응할 수는 있게 된다. 
